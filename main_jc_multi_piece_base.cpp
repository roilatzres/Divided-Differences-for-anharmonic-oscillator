            ...
            for (int target = 0; target < max_target.cavity; target++) {
                ...
                for(int q = 0; q <= q_max; q++){
                    ...
                    if (target == start_state.cavity && qubit == start_state.qubit && q == 0){
                        ...
                        // complex<double> exponent = std::exp(-i_num * (double) (t * start_state.cavity * chi)) ;
                        complex<double> exponent = std::exp(-i_num * (double)0);
                    ...
                    std::string q_amp_dir = "./include/q_amp/jc_base/"
                    ...
                    for (const auto& coefficient : all_coefficients_const_pulse) {
                        complex_Ex res = cal_divdiff_jc_base(coefficient, t, q, qubit, chi, alpha, delta);//TODO: check if func is correct
...
void save_all_state_ta_to_json(
    ...
    amp_stream << std::scientific << amplitude;
    std::string amp_str = amp_stream.str();
    ...
    for(int pulse = 0; pulse < num_pulses; pulse++){
        ...
        for(int qubit = 0; qubit < max_target_qubit; qubit++){
            ...
            std::string filename;
            if (qubit == 0){
                filename = "jc_runs/amplitudes_g_chg_pulse" + std::to_string(pulse) + "amp" + amp_str + "_q" + std::to_string(q_max) + "_nc" + std::to_string(max_target_cavity) + "_nq" + std::to_string(max_target_qubit) + "_p" + std::to_string(num_pulses) + ".json";
            }else if (qubit == 1){
                filename = "jc_runs/amplitudes_e_chg_pulse" + std::to_string(pulse) + "amp" + amp_str + "_q" + std::to_string(q_max) + "_nc" + std::to_string(max_target_cavity) + "_nq" + std::to_string(max_target_qubit) + "_p" + std::to_string(num_pulses) + ".json";
            }
            std::ofstream o(filename);
            o << j << std::endl;
        }
...
int main(int argc, char* argv[]) {
    ...
    double coupling = sqrt(abs(delta * chi));
    //print coupling
