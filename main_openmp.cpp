        ...
        // for (int i = 0; i < times.size(); i++) { 
            ...
            for(int target=0; target <= target_max; target++){
                ...
                for(int q = 0; q <= q_max; q++){
                    // all_transition_amplitudes[qubit][t][target][q] = mpi_compute_q_amplitude(target, q, qubit, amplitude, t, chi, omega, total_amp, all_coefficients);
                    ...
                    all_transition_amplitudes[qubit][t][target][q] = local_amp;
                ...
                all_transition_amplitudes[qubit][t][target][0] = total_amp;
            ...
            for(int target=0; target <= target_max; target++){
                all_transition_amplitudes[qubit][t][target][0] /= norm;
            }
            // }//pragma target
    ...
    for (int qubit_p = 0; qubit_p < all_transition_amplitudes.size(); qubit_p++) {
        ...
        for (int time_p = 0; time_p < all_transition_amplitudes[qubit_p].size(); time_p++) {
            ...
            for(int target_p = 0; target_p <= target_max; target_p++){
                complex<double> final_amp = all_transition_amplitudes[qubit_p][time_p][target_p][0];
                ...
                //             cout << "q_amp: " << 0 << endl;
                //         }
                //         cout << "Q_amp: " << all_transition_amplitudes[qubit_p][time_p][target_p][q_amp_p] << endl;
