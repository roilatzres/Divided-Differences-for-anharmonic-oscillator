        ...
        //     // t = 96;
            ...
            for (int target = 0; target < max_target; target++) {
                ...
                for(int q = 0; q <= q_max; q++){
                    ...
                    std::string q_amp_filename = "./include/q_amp/dispersive_const/dispersive_q_amp_start_state_" + std::to_string(start_state) 
...
int main(int argc, char* argv[]) {
    ...
    for (int qubit = 0; qubit < 2; qubit++){
        ...
        for (int i = 0; i < (final_t - mid_t); i++){// times size is 1
    ...
    for (int start_state = 0; start_state < max_target; start_state++){
        ...
        curr_state = solve_for_t(final_t - mid_t, q_max, (-1) * amplitude, start_state, max_target);//TODO: check t - 192/96/95?
        ...
        for(int qubit = 0; qubit < 2; qubit++){
            ...
            for(int i=0; i < int(mid_t); i++){
                ...
                for (int j = 0; j < max_target; j++){//curr_state_ta[qubit][0] refers to time 96
                    ...
                    curr_state_ta[start_state][qubit][i][j] = complex_mult(curr_state_ta[start_state][qubit][i][j], mid_ta[qubit][mid_t - 1][start_state]);
                    ...
                    divdiff_init();
                    final_state_ta[qubit][i][j].real += curr_state_ta[start_state][qubit][i][j].real;
                    final_state_ta[qubit][i][j].imag += curr_state_ta[start_state][qubit][i][j].imag;
                    divdiff_clear_up();

                    // //print final_state_ta
                    // std::cout << "final_state_ta:" << std::endl;
                    // print_complex_Ex(final_state_ta[qubit][i][j]);
